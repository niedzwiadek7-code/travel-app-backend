...
export class Activity1709552798273 implements Seeder {
  ...
  public async run(
    ...
    await dataSource.query(`TRUNCATE TABLE "${process.env.DATABASE_SCHEMA}"."activity_entity" RESTART IDENTITY CASCADE`)
    await dataSource.query(`TRUNCATE TABLE "${process.env.DATABASE_SCHEMA}"."price_entity" RESTART IDENTITY CASCADE`)
    ...
    const activityTypeParametersRepository = dataSource.getRepository(ActivityTypeParameterEntity)
